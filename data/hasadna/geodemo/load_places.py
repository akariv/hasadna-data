...
RADIUSES = [ 100, 200, 500 ]

results = {}
for lat in LAT:
    for lon in LON:
        for radius in RADIUSES:
            res = urllib2.urlopen("https://maps.googleapis.com/maps/api/place/search/json?location=%f,%f&radius=%s&sensor=false&key=AIzaSyAwwAQHYJ29SkL3Ipi9JO-15mR5OXNIUl0" % (lat,lon,radius) ).read()
            res = json.loads(res)
            try:
